...
/*** Rights ***/
// Generally for people with the right to view mail_letter
$groupID = 1024;
GRGR($groupID);

$_SESSION['entity'] = 'mail_letter';
...
$n4a['mail_letter_d.php'] = '' . ss('Add Mail_letter') . '';
if (!$headless) require("inc/header.inc.php");

$autocomplete['pdf']['dbTable'] = 'pdf';
$autocomplete['pdf']['firstVarchar'] = 'name';
...
$_SESSION[$modul]['title'] = $_VALID['title'];
...
// Pdf
validate('pdf', 'string nullable' );
$_SESSION[$modul]['pdf'] = $_VALID['pdf'];
...
// where condition
if ($_VALID['name']) {
	$where[] = "mail_letter.name LIKE '%" . mysqli_real_escape_string($con, $_VALID['name']) . "%'";
}
$_SESSION[$modul]['name'] = $_VALID['name'];
if ($_VALID['title']) {
	$where[] = "mail_letter.title LIKE '%" . mysqli_real_escape_string($con, $_VALID['title']) . "%'";
}
$_SESSION[$modul]['title'] = $_VALID['title'];
if ($_VALID['pdf']) {
	$where[] = "pdf.name LIKE '%" . mysqli_real_escape_string($con, $_VALID['pdf']) . "%'";
}
$_SESSION[$modul]['pdf'] = $_VALID['pdf'];
$where = ($where) ? implode(" AND ", $where) : "1=1";
//List Hook After Where
$sql = "SELECT mail_letter.mail_letter_id,
mail_letter.name,
mail_letter.title,
mail_letter.html,
pdf.name as pdf
...
<section class="panel">
    <header class="panel-heading">
        ...
        <h2 class="panel-title"><?php echo ss('Mail_letter'); ?></h2>
    </header>
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="mb-md text-left">
                    <a class="btn btn-success" href="mail_letter_d.php" title=" <?php echo ss('Add new'); ?>"><i class="fa fa-plus"></i> <?php echo ss('Add new'); ?></a>
                    <!-- Here goes nothing -->
                </div>
            </div>
            <div class="col-sm-12 col-md-6">
...
if (!$headless) {
    ...
    // title title
    echo '<th class="hasmenu grey" id="th_title" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_title" onClick="changeSort(\'title\')">' . ss('Title') . '</a>
    </th>';
        
    // title pdf
    echo '<th class="hasmenu grey" id="th_pdf" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_pdf" onClick="changeSort(\'pdf\')">' . ss('Pdf') . '</a>
    </th>';
    ...
    // filter title
    echo '<th nowrap>
    <input class="form-control search" name="title" id="title" value="'.$_SESSION[$modul]['title'].'" autocomplete="off"></th>';

    // filter pdf
    echo '<th nowrap>
    <input class="form-control search" name="pdf" id="pdf" value="'.$_SESSION[$modul]['pdf'].'" autocomplete="off"></th>';
...
if (!$listResult) {
    ...
    while($row = mysqli_fetch_array($listResult)) {
        echo '<tr class="dotted ' .  ((($i++ % 2)==0) ? "tr_even":"tr_odd") . '" id="tr_'.$row['mail_letter_id'].'">';
        echo '<td nowrap class="text-center actions-hover actions-fade">
        <a href="mail_letter_d.php?i=' . $i . '&amp;mail_letter_id=' . (int) $row['mail_letter_id'] . '" title="' . ss('Edit') . '" data-toggle="tooltip" data-placement="top">
        <i style="font-size:20px" class="fa fa-pencil" ></i>
        </a>';
        // people with right to delete see the delete button
        if (R(3)){
            echo '&nbsp;&nbsp;<a href="#" title="' . ss('Delete') . '" onclick="delRow(\''.$row['mail_letter_id'].'\');" data-toggle="tooltip" data-placement="top">
            <i class="fa fa-trash-o" style="color:red;font-size:20px;"></i></a>';
        }
        echo '</td>';

        // name
        echo '<td onClick="location.href=\'mail_letter_d.php?i='.($i-1).'&amp;mail_letter_id='.$row['mail_letter_id'].'\'" nowrap>
        ' . str_limit(htmlspecialchars($row['name'])) . '
        </td>';

        // title
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['title'])) . '
        </td>';

        // pdf
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['pdf'])) . '
        </td>';
    }
}
            ...
            <div class="panel-body">
                <div class="modal-wrapper">
                    ...
                    <div class="modal-text">
                        <p><?php echo ss('Do you really want to delete the Mail_letter?'); ?></p>
                    </div>
                </div>
            </div>
...
$(function(){
    $(document).contextmenu({
        ...
        select: function(event, ui) {
            ...
            switch(ui.cmd){
                case "add":
                    window.location.href = 'red_button_entity_d.php?return_script=<?php echo urlencode($_SERVER['PHP_SELF'].'?BRB')?>&entity_name=mail_letter&after=' + $target.uniqueId().attr('id').substring(3);
                    break
                case "delete":
                    $.ajax({
                        url: 'a/red_button_entity_del.php?searchid&entity_name=mail_letter&data_name='+$target.uniqueId().attr('id').substring(3)
                    }).done( function( data ) {
                        $.ajax({
                            url: 'bigredbutton.php?array=mail_letter&http_method=_REQUEST&is_code_type=0&type=form&is_file=0&hrose_write_file=on&hrose_overwrite_file=on&hrose_overwrite_changed_file=on&is_options=0&charset=UTF8&trans=on&mysql_typecast=on&mysql_full=on&validate_store_session=on&type_filter=on&custom_html_tags&form_full=on&list_search=on&list_sort=on&list_limit=on&list_delete=on&list_ajax=on&dynamic_list_header=on&list_odd_row=on&submitted=yes'
                        }).done( function( data ) {
                            $.ajax({
                                url: 'bigredbutton.php?array=mail_letter&http_method=_REQUEST&is_code_type=0&type=list&is_file=0&hrose_write_file=on&hrose_overwrite_file=on&hrose_overwrite_changed_file=on&is_options=0&charset=UTF8&trans=on&mysql_typecast=on&mysql_full=on&validate_store_session=on&type_filter=on&custom_html_tags&form_full=on&list_search=on&list_sort=on&list_limit=on&list_delete=on&list_ajax=on&dynamic_list_header=on&list_odd_row=on&submitted=yes'
                            }).done( function( data ) {
                                location.reload();
                                return false;
                            });
    ...
    function delRow(pk) {
        ...
        $(document).on('click', '#modalDelete .modal-confirm', function (e) {
            e.preventDefault();
            fetch('mail_letter_d.php?delete=1&mail_letter_id='+pk)
            .then(function(response){
                $('#tr_'+pk).hide();
                new PNotify({
                    title: 'Success!',
                    text: '<?php echo ss('The specific mail_letter deleted!'); ?>',
                    type: 'success'
                });
                $(document).off('click','#modalDelete .modal-dismiss');
                $(document).off('click','#modalDelete .modal-confirm');
...
function autocompleteUpdate(elem) {
    ...
    $.ajax({
    url: 'mail_letter_d.php?list_update',
    type: 'post',
    data: { field:field_name, [field_name]:value, mail_letter_id:edit_id },
  ...
  $(".dotted td").on("click", function() {
    ...
    $( ".autocomplete" ).autocomplete({
       source: function( request , response ) {
        ...
        $.ajax({
            url: "mail_letter.php?headless=1&autocomplete",
            data : param,
            dataType: "json",
            type: "GET",
