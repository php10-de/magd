...
/*** Rights ***/
// Generally for people with the right to view mail_sender
$groupID = 1024;
GRGR($groupID);

$_SESSION['entity'] = 'mail_sender';
...
$orderBy = ($_VALID['sortcol'])?$_VALID['sortcol'] .(($_VALID['sortdir'])?' ' . $_VALID['sortdir']:''):'mail_sender_id';
$orderBy = mysqli_real_escape_string ( $con , $orderBy );
    
$headless = isset($_REQUEST['headless']);
$n4a['mail_sender_d.php'] = '' . ss('Add Mail_sender') . '';
...
// Mail_sender_id
validate('mail_sender_id', 'int nullable' );
$_SESSION[$modul]['mail_sender_id'] = $_VALID['mail_sender_id'];

// From_mail
validate('from_mail', 'string' );
$_SESSION[$modul]['from_mail'] = $_VALID['from_mail'];

// From_name
validate('from_name', 'string' );
$_SESSION[$modul]['from_name'] = $_VALID['from_name'];

// Response_mail
validate('response_mail', 'string' );
$_SESSION[$modul]['response_mail'] = $_VALID['response_mail'];

// Response_name
validate('response_name', 'string' );
$_SESSION[$modul]['response_name'] = $_VALID['response_name'];
...
// where condition
if ($_VALID['from_mail']) {
	$where[] = "mail_sender.from_mail LIKE '%" . mysqli_real_escape_string($con, $_VALID['from_mail']) . "%'";
}
$_SESSION[$modul]['from_mail'] = $_VALID['from_mail'];
if ($_VALID['from_name']) {
	$where[] = "mail_sender.from_name LIKE '%" . mysqli_real_escape_string($con, $_VALID['from_name']) . "%'";
}
$_SESSION[$modul]['from_name'] = $_VALID['from_name'];
if ($_VALID['response_mail']) {
	$where[] = "mail_sender.response_mail LIKE '%" . mysqli_real_escape_string($con, $_VALID['response_mail']) . "%'";
}
$_SESSION[$modul]['response_mail'] = $_VALID['response_mail'];
if ($_VALID['response_name']) {
	$where[] = "mail_sender.response_name LIKE '%" . mysqli_real_escape_string($con, $_VALID['response_name']) . "%'";
}
$_SESSION[$modul]['response_name'] = $_VALID['response_name'];
$where = ($where) ? implode(" AND ", $where) : "1=1";
//List Hook After Where
$sql = "SELECT mail_sender.mail_sender_id,
mail_sender.from_mail,
mail_sender.from_name,
mail_sender.response_mail,
mail_sender.response_name
FROM `mail_sender`
WHERE " . $where . "
ORDER BY " . $orderBy . "
" . (($_VALID['limit']) ? "
...
<section class="panel">
    <header class="panel-heading">
        ...
        <h2 class="panel-title"><?php echo ss('Mail_sender'); ?></h2>
    </header>
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="mb-md text-left">
                    <a class="btn btn-success" href="mail_sender_d.php" title=" <?php echo ss('Add new'); ?>"><i class="fa fa-plus"></i> <?php echo ss('Add new'); ?></a>
                    <!-- Here goes nothing -->
                </div>
            </div>
            <div class="col-sm-12 col-md-6">
...
if (!$headless) {
    ...
    // title from_mail
    echo '<th class="hasmenu grey" id="th_from_mail" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_from_mail" onClick="changeSort(\'from_mail\')">' . ss('From_mail') . '</a>
    </th>';
        
    // title from_name
    echo '<th class="hasmenu grey" id="th_from_name" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_from_name" onClick="changeSort(\'from_name\')">' . ss('From_name') . '</a>
    </th>';
        
    // title response_mail
    echo '<th class="hasmenu grey" id="th_response_mail" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_response_mail" onClick="changeSort(\'response_mail\')">' . ss('Response_mail') . '</a>
    </th>';
        
    // title response_name
    echo '<th class="hasmenu grey" id="th_response_name" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_response_name" onClick="changeSort(\'response_name\')">' . ss('Response_name') . '</a>
    </th>';
    ...
    // filter from_mail
    echo '<th nowrap>
    <input class="form-control search" name="from_mail" id="from_mail" value="'.$_SESSION[$modul]['from_mail'].'" autocomplete="off"></th>';

    // filter from_name
    echo '<th nowrap>
    <input class="form-control search" name="from_name" id="from_name" value="'.$_SESSION[$modul]['from_name'].'" autocomplete="off"></th>';

    // filter response_mail
    echo '<th nowrap>
    <input class="form-control search" name="response_mail" id="response_mail" value="'.$_SESSION[$modul]['response_mail'].'" autocomplete="off"></th>';

    // filter response_name
    echo '<th nowrap>
    <input class="form-control search" name="response_name" id="response_name" value="'.$_SESSION[$modul]['response_name'].'" autocomplete="off"></th>';
...
if (!$listResult) {
    ...
    while($row = mysqli_fetch_array($listResult)) {
        echo '<tr class="dotted ' .  ((($i++ % 2)==0) ? "tr_even":"tr_odd") . '" id="tr_'.$row['mail_sender_id'].'">';
        echo '<td nowrap class="text-center actions-hover actions-fade">
        <a href="mail_sender_d.php?i=' . $i . '&amp;mail_sender_id=' . (int) $row['mail_sender_id'] . '" title="' . ss('Edit') . '" data-toggle="tooltip" data-placement="top">
        <i style="font-size:20px" class="fa fa-pencil" ></i>
        </a>';
        // people with right to delete see the delete button
        if (R(3)){
            echo '&nbsp;&nbsp;<a href="#" title="' . ss('Delete') . '" onclick="delRow(\''.$row['mail_sender_id'].'\');" data-toggle="tooltip" data-placement="top">
            <i class="fa fa-trash-o" style="color:red;font-size:20px;"></i></a>';
        }
        echo '</td>';

        // from_mail
        echo '<td onClick="location.href=\'mail_sender_d.php?i='.($i-1).'&amp;mail_sender_id='.$row['mail_sender_id'].'\'" nowrap>
        ' . str_limit(htmlspecialchars($row['from_mail'])) . '
        </td>';

        // from_name
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['from_name'])) . '
        </td>';

        // response_mail
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['response_mail'])) . '
        </td>';

        // response_name
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['response_name'])) . '
        </td>';
    }
}
            ...
            <div class="panel-body">
                <div class="modal-wrapper">
                    ...
                    <div class="modal-text">
                        <p><?php echo ss('Do you really want to delete the Mail_sender?'); ?></p>
                    </div>
                </div>
            </div>
...
$(function(){
    ...
    function delRow(pk) {
        ...
        $(document).on('click', '#modalDelete .modal-confirm', function (e) {
            e.preventDefault();
            fetch('mail_sender_d.php?delete=1&mail_sender_id='+pk)
            .then(function(response){
                $('#tr_'+pk).hide();
                new PNotify({
                    title: 'Success!',
                    text: '<?php echo ss('The specific mail_sender deleted!'); ?>',
                    type: 'success'
                });
                $(document).off('click','#modalDelete .modal-dismiss');
                $(document).off('click','#modalDelete .modal-confirm');
...
function autocompleteUpdate(elem) {
    ...
    $.ajax({
    url: 'mail_sender_d.php?list_update',
    type: 'post',
    data: { field:field_name, [field_name]:value, mail_sender_id:edit_id },
  ...
  $(".dotted td").on("click", function() {
    ...
    $( ".autocomplete" ).autocomplete({
       source: function( request , response ) {
        ...
        $.ajax({
            url: "mail_sender.php?headless=1&autocomplete",
            data : param,
            dataType: "json",
            type: "GET",
