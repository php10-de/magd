<?php namespace module\imap;

use module\imap\IncomingMail;
use module\imap\IncomingMailAttachment;
use module\imap\IncomingMailHeader;
use stdClass;

require_once 'IncomingMail.php';
require_once 'IncomingMailHeader.php';
require_once 'IncomingMailAttachment.php';
...
class Mailbox {
    ...
    public function __construct($imapPath, $login, $password, $attachmentsDir = null, $serverEncoding = 'UTF-8') {
        ...
        if($attachmentsDir) {
            if(!is_dir($attachmentsDir)) {
                throw new \PhpImap\Exception('Directory "' . $attachmentsDir . '" not found');
            }
    ...
    protected function initImapStreamWithRetry() {
        ...
        do {
            ...
            catch(\PhpImap\ConnectionException $exception) {
            }
        }
    ...
    protected function initImapStream() {
        ...
        return $this->imap('open', [$this->imapPath, $this->imapLogin, $this->imapPassword, $this->imapOptions, $this->imapRetriesNum, $this->imapParams], false, \PhpImap\ConnectionException::class);
    }
    ...
    protected function convertStringEncoding($string, $fromEncoding, $toEncoding) {
        ...
        if(!$convertedString) {
            throw new \PhpImap\Exception('Mime string encoding conversion failed');
        }
     ...
     * @throws \PhpImap\Exception
     */
    public function imap($methodShortName, $args = [], $prependConnectionAsFirstArg = true, $throwExceptionClass = \PhpImap\Exception::class) {
        if(!is_array($args)) {
            $args = [$args];
        }
        foreach($args as &$arg) {
