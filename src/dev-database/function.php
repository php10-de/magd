...
/*** Rights ***/
// Generally for people with the right to view function
$groupID = 1001;
GRGR($groupID);

$_SESSION['entity'] = 'function';
...
$n4a['function_d.php'] = '' . ss('Add Function') . '';
/*** After Navigation Entries ***/
if (!$headless) require("inc/header.inc.php");

$autocomplete['ai']['dbTable'] = 'ai';
$autocomplete['ai']['firstVarchar'] = 'name';
...
// Function_id
validate('function_id', 'int nullable' );
$_SESSION[$modul]['function_id'] = $_VALID['function_id'];
...
$_SESSION[$modul]['description'] = $_VALID['description'];

// Required
validate('required', 'string nullable' );
$_SESSION[$modul]['required'] = $_VALID['required'];
...
// where condition
if ($_VALID['ai']) {
	$where[] = "ai.name LIKE '%" . mysqli_real_escape_string($con, $_VALID['ai']) . "%'";
}
$_SESSION[$modul]['ai'] = $_VALID['ai'];
if ($_VALID['name']) {
	$where[] = "function.name LIKE '%" . mysqli_real_escape_string($con, $_VALID['name']) . "%'";
}
$_SESSION[$modul]['name'] = $_VALID['name'];
if ($_VALID['description']) {
	$where[] = "function.description LIKE '%" . mysqli_real_escape_string($con, $_VALID['description']) . "%'";
}
$_SESSION[$modul]['description'] = $_VALID['description'];
if ($_VALID['required']) {
	$where[] = "function.required LIKE '%" . mysqli_real_escape_string($con, $_VALID['required']) . "%'";
}
$_SESSION[$modul]['required'] = $_VALID['required'];
$where = ($where) ? implode(" AND ", $where) : "1=1";
//List Hook After Where
$sql = "SELECT function.function_id,
ai.name as ai,
function.name,
...
<section class="panel">
    <header class="panel-heading">
        ...
        <h2 class="panel-title"><?php echo ss('Function'); ?></h2>
    </header>
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="mb-md text-left">
                    <a class="btn btn-success" href="function_d.php" title=" <?php echo ss('Add new'); ?>"><i class="fa fa-plus"></i> <?php echo ss('Add new'); ?></a>
                    <!-- Here goes nothing -->
                </div>
            </div>
            <div class="col-sm-12 col-md-6">
...
if (!$headless) {
    ...
    // title description
    echo '<th class="hasmenu grey" id="th_description" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_description" onClick="changeSort(\'description\')">' . ss('Description') . '</a>
    </th>';
        
    // title required
    echo '<th class="hasmenu grey" id="th_required" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_required" onClick="changeSort(\'required\')">' . ss('Required') . '</a>
    </th>';
    ...
    // filter description
    echo '<th nowrap>
    <input class="form-control search" name="description" id="description" value="'.$_SESSION[$modul]['description'].'" autocomplete="off"></th>';

    // filter required
    echo '<th nowrap>
    <input class="form-control search" name="required" id="required" value="'.$_SESSION[$modul]['required'].'" autocomplete="off"></th>';
...
if (!$listResult) {
    ...
    while($row = mysqli_fetch_array($listResult)) {
        echo '<tr class="dotted ' .  ((($i++ % 2)==0) ? "tr_even":"tr_odd") . '" id="tr_'.$row['function_id'].'">';
        echo '<td nowrap class="text-center actions-hover actions-fade">
        <a href="function_d.php?i=' . $i . '&amp;function_id=' . (int) $row['function_id'] . '" title="' . ss('Edit') . '" data-toggle="tooltip" data-placement="top">
        <i style="font-size:20px" class="fa fa-pencil" ></i>
        </a>';
        // people with right to delete see the delete button
        if (R(3)){
            echo '&nbsp;&nbsp;<a href="#" title="' . ss('Delete') . '" onclick="delRow(\''.$row['function_id'].'\');" data-toggle="tooltip" data-placement="top">
            <i class="fa fa-trash-o" style="color:red;font-size:20px;"></i></a>';
        }
        echo '</td>';

        // ai
        echo '<td onClick="location.href=\'function_d.php?i='.($i-1).'&amp;function_id='.$row['function_id'].'\'" nowrap>
        ...
        // description
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['description'])) . '
        </td>';

        // required
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['required'])) . '
        </td>';
    }
}
            ...
            <div class="panel-body">
                <div class="modal-wrapper">
                    ...
                    <div class="modal-text">
                        <p><?php echo ss('Do you really want to delete the Function?'); ?></p>
                    </div>
                </div>
            </div>
...
$(function(){
    $(document).contextmenu({
        ...
        select: function(event, ui) {
            ...
            switch(ui.cmd){
                case "add":
                    window.location.href = 'red_button_entity_d.php?return_script=<?php echo urlencode($_SERVER['PHP_SELF'].'?BRB')?>&entity_name=function&after=' + $target.uniqueId().attr('id').substring(3);
                    break
                case "delete":
                    $.ajax({
                        url: 'a/red_button_entity_del.php?searchid&entity_name=function&data_name='+$target.uniqueId().attr('id').substring(3)
                    }).done( function( data ) {
                        $.ajax({
                            url: 'bigredbutton.php?array=function&http_method=_REQUEST&is_code_type=0&type=form&is_file=0&hrose_write_file=on&hrose_overwrite_file=on&hrose_overwrite_changed_file=on&is_options=0&charset=UTF8&trans=on&mysql_typecast=on&mysql_full=on&validate_store_session=on&type_filter=on&custom_html_tags&form_full=on&list_search=on&list_sort=on&list_limit=on&list_delete=on&list_ajax=on&dynamic_list_header=on&list_odd_row=on&submitted=yes'
                        }).done( function( data ) {
                            $.ajax({
                                url: 'bigredbutton.php?array=function&http_method=_REQUEST&is_code_type=0&type=list&is_file=0&hrose_write_file=on&hrose_overwrite_file=on&hrose_overwrite_changed_file=on&is_options=0&charset=UTF8&trans=on&mysql_typecast=on&mysql_full=on&validate_store_session=on&type_filter=on&custom_html_tags&form_full=on&list_search=on&list_sort=on&list_limit=on&list_delete=on&list_ajax=on&dynamic_list_header=on&list_odd_row=on&submitted=yes'
                            }).done( function( data ) {
                                location.reload();
                                return false;
                            });
    ...
    function delRow(pk) {
        ...
        $(document).on('click', '#modalDelete .modal-confirm', function (e) {
            e.preventDefault();
            fetch('function_d.php?delete=1&function_id='+pk)
            .then(function(response){
                $('#tr_'+pk).hide();
                new PNotify({
                    title: 'Success!',
                    text: '<?php echo ss('The specific function deleted!'); ?>',
                    type: 'success'
                });
                $(document).off('click','#modalDelete .modal-dismiss');
                $(document).off('click','#modalDelete .modal-confirm');
...
function autocompleteUpdate(elem) {
    ...
    $.ajax({
    url: 'function_d.php?list_update',
    type: 'post',
    data: { field:field_name, [field_name]:value, function_id:edit_id },
  ...
  $(".dotted td").on("click", function() {
    ...
    $( ".autocomplete" ).autocomplete({
       source: function( request , response ) {
        ...
        $.ajax({
            url: "function.php?headless=1&autocomplete",
            data : param,
            dataType: "json",
            type: "GET",
