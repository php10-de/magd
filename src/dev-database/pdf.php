...
/*** Rights ***/
// Generally for people with the right to view pdf
$groupID = 1001;
GRGR($groupID);
...
// Pdf_id
validate('pdf_id', 'int nullable' );
$_SESSION[$modul]['pdf_id'] = $_VALID['pdf_id'];
...
$_SESSION[$modul]['html_template'] = $_VALID['html_template'];

// Static_file_media
validate('static_file_media', 'media nullable' );
$_SESSION[$modul]['static_file_media'] = $_VALID['static_file_media'];
...
// where condition
if ($_VALID['name']) {
	$where[] = "pdf.name LIKE '%" . mysqli_real_escape_string($con, $_VALID['name']) . "%'";
}
$_SESSION[$modul]['name'] = $_VALID['name'];
if ($_VALID['html_template']) {
	$where[] = "pdf.html_template LIKE '%" . mysqli_real_escape_string($con, $_VALID['html_template']) . "%'";
}
$_SESSION[$modul]['html_template'] = $_VALID['html_template'];
if ($_VALID['static_file_media']) {
	$where[] = "pdf.static_file_media = '" . mysqli_real_escape_string($con, $_VALID['static_file_media']) . "'";
}
$_SESSION[$modul]['static_file_media'] = $_VALID['static_file_media'];
$where = ($where) ? implode(" AND ", $where) : "1=1";
//List Hook After Where
$sql = "SELECT pdf.pdf_id,
pdf.name,
pdf.html_template,
pdf.static_file_media
FROM `pdf`
WHERE " . $where . "
ORDER BY " . $orderBy . "
" . (($_VALID['limit']) ? "
...
<section class="panel">
    <header class="panel-heading">
        ...
        <h2 class="panel-title"><?php echo ss('Pdf'); ?></h2>
    </header>
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="mb-md text-left">
                    <a class="btn btn-success" href="pdf_d.php" title=" <?php echo ss('Add new'); ?>"><i class="fa fa-plus"></i> <?php echo ss('Add new'); ?></a>
                    <!-- Here goes nothing -->
                </div>
            </div>
            <div class="col-sm-12 col-md-6">
...
if (!$headless) {
    ...
    // title html_template
    echo '<th class="hasmenu grey" id="th_html_template" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_html_template" onClick="changeSort(\'html_template\')">' . ss('Html_template') . '</a>
    </th>';
        
    // title static_file_media
    echo '<th class="hasmenu grey" id="th_static_file_media" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_static_file_media" onClick="changeSort(\'static_file_media\')">' . ss('Static_file_media') . '</a>
    </th>';
    ...
    echo '<th nowrap>
    <input class="form-control search" name="html_template" id="html_template" value="'.$_SESSION[$modul]['html_template'].'" autocomplete="off"></th>';

    // filter static_file_media
    echo '<th nowrap>
    <input class="form-control search" name="static_file_media" id="static_file_media" value="'.$_SESSION[$modul]['static_file_media'].'" autocomplete="off"></th>';
...
if (!$listResult) {
    ...
    while($row = mysqli_fetch_array($listResult)) {
        echo '<tr class="dotted ' .  ((($i++ % 2)==0) ? "tr_even":"tr_odd") . '" id="tr_'.$row['pdf_id'].'">';
        echo '<td nowrap class="text-center actions-hover actions-fade">
        <a href="pdf_d.php?i=' . $i . '&amp;pdf_id=' . (int) $row['pdf_id'] . '" title="' . ss('Edit') . '" data-toggle="tooltip" data-placement="top">
        <i style="font-size:20px" class="fa fa-pencil" ></i>
        </a>';
        // people with right to delete see the delete button
        if (R(3)){
            echo '&nbsp;&nbsp;<a href="#" title="' . ss('Delete') . '" onclick="delRow(\''.$row['pdf_id'].'\');" data-toggle="tooltip" data-placement="top">
            <i class="fa fa-trash-o" style="color:red;font-size:20px;"></i></a>';
        }
        echo '</td>';

        // name
        echo '<td onClick="location.href=\'pdf_d.php?i='.($i-1).'&amp;pdf_id='.$row['pdf_id'].'\'" nowrap>
        ' . str_limit(htmlspecialchars($row['name'])) . '
        </td>';

        // html_template
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['html_template'])) . '
        </td>';

        // static_file_media
            $media = '&nbsp;';
            $mediaUrl = HTTP_HOST . 'pdf_d.php?show_media&filename=' . $row['static_file_media'];
            if (strpos($row['static_file_media'], '.pdf') !== false) {
                $media = '<a href="' . $mediaUrl . '" target="_blank"><i class="fa fa-file-pdf-o"></i></a>';
            } elseif($row['static_file_media']) {
            ...
            <div class="panel-body">
                <div class="modal-wrapper">
                    ...
                    <div class="modal-text">
                        <p><?php echo ss('Do you really want to delete the Pdf?'); ?></p>
                    </div>
                </div>
            </div>
...
$(function(){
    $(document).contextmenu({
        ...
        select: function(event, ui) {
            ...
            switch(ui.cmd){
                case "add":
                    window.location.href = 'red_button_entity_d.php?return_script=<?php echo urlencode($_SERVER['PHP_SELF'].'?BRB')?>&entity_name=pdf&after=' + $target.uniqueId().attr('id').substring(3);
                    break
                case "delete":
                    $.ajax({
                        url: 'a/red_button_entity_del.php?searchid&entity_name=pdf&data_name='+$target.uniqueId().attr('id').substring(3)
                    }).done( function( data ) {
                        $.ajax({
                            url: 'bigredbutton.php?array=pdf&http_method=_REQUEST&is_code_type=0&type=form&is_file=0&hrose_write_file=on&hrose_overwrite_file=on&hrose_overwrite_changed_file=on&is_options=0&charset=UTF8&trans=on&mysql_typecast=on&mysql_full=on&validate_store_session=on&type_filter=on&custom_html_tags&form_full=on&list_search=on&list_sort=on&list_limit=on&list_delete=on&list_ajax=on&dynamic_list_header=on&list_odd_row=on&submitted=yes'
                        }).done( function( data ) {
                            $.ajax({
                                url: 'bigredbutton.php?array=pdf&http_method=_REQUEST&is_code_type=0&type=list&is_file=0&hrose_write_file=on&hrose_overwrite_file=on&hrose_overwrite_changed_file=on&is_options=0&charset=UTF8&trans=on&mysql_typecast=on&mysql_full=on&validate_store_session=on&type_filter=on&custom_html_tags&form_full=on&list_search=on&list_sort=on&list_limit=on&list_delete=on&list_ajax=on&dynamic_list_header=on&list_odd_row=on&submitted=yes'
                            }).done( function( data ) {
                                location.reload();
                                return false;
                            });
    ...
    function delRow(pk) {
        ...
        $(document).on('click', '#modalDelete .modal-confirm', function (e) {
            e.preventDefault();
            fetch('pdf_d.php?delete=1&pdf_id='+pk)
            .then(function(response){
                $('#tr_'+pk).hide();
                new PNotify({
                    title: 'Success!',
                    text: '<?php echo ss('The specific pdf deleted!'); ?>',
                    type: 'success'
                });
                $(document).off('click','#modalDelete .modal-dismiss');
                $(document).off('click','#modalDelete .modal-confirm');
...
function autocompleteUpdate(elem) {
    ...
    $.ajax({
    url: 'pdf_d.php?list_update',
    type: 'post',
    data: { field:field_name, [field_name]:value, pdf_id:edit_id },
  ...
  $(".dotted td").on("click", function() {
    ...
    $( ".autocomplete" ).autocomplete({
       source: function( request , response ) {
        ...
        $.ajax({
            url: "pdf.php?headless=1&autocomplete",
            data : param,
            dataType: "json",
            type: "GET",
