...
/*** Rights ***/
// Generally for people with the right to view red_button_conf
$groupID = 29;
GRGR($groupID);

$_SESSION['entity'] = 'red_button_conf';
...
$orderBy = ($_VALID['sortcol'])?$_VALID['sortcol'] .(($_VALID['sortdir'])?' ' . $_VALID['sortdir']:''):'red_button_conf_id';
$orderBy = mysqli_real_escape_string ( $con , $orderBy );
    
$headless = isset($_REQUEST['headless']);
$n4a['red_button_conf_d.php'] = '' . ss('Add Red_button_conf') . '';
...
$_SESSION[$modul]['red_button_conf_id'] = $_VALID['red_button_conf_id'];

// Red_button_entity_name
validate('red_button_entity_name', 'string' );
$_SESSION[$modul]['red_button_entity_name'] = $_VALID['red_button_entity_name'];

// Red_button_data_name
validate('red_button_data_name', 'string nullable' );
$_SESSION[$modul]['red_button_data_name'] = $_VALID['red_button_data_name'];

// Param
validate('param', 'string' );
$_SESSION[$modul]['param'] = $_VALID['param'];
...
// where condition
if ($_VALID['red_button_conf_id']) {
	$where[] = "red_button_conf.red_button_conf_id =  " . $_VALIDDB['red_button_conf_id'];
}
$_SESSION[$modul]['red_button_conf_id'] = $_VALID['red_button_conf_id'];
if ($_VALID['red_button_entity_name']) {
	$where[] = "red_button_conf.red_button_entity_name LIKE '%" . mysqli_real_escape_string($con, $_VALID['red_button_entity_name']) . "%'";
}
$_SESSION[$modul]['red_button_entity_name'] = $_VALID['red_button_entity_name'];
if ($_VALID['red_button_data_name']) {
	$where[] = "red_button_conf.red_button_data_name LIKE '%" . mysqli_real_escape_string($con, $_VALID['red_button_data_name']) . "%'";
}
$_SESSION[$modul]['red_button_data_name'] = $_VALID['red_button_data_name'];
if ($_VALID['param']) {
	$where[] = "red_button_conf.param LIKE '%" . mysqli_real_escape_string($con, $_VALID['param']) . "%'";
}
$_SESSION[$modul]['param'] = $_VALID['param'];
if ($_VALID['value']) {
	$where[] = "red_button_conf.value LIKE '%" . mysqli_real_escape_string($con, $_VALID['value']) . "%'";
}
$_SESSION[$modul]['value'] = $_VALID['value'];
$where = ($where) ? implode(" AND ", $where) : "1=1";
//List Hook After Where
$sql = "SELECT red_button_conf.red_button_conf_id,
red_button_conf.red_button_entity_name,
red_button_conf.red_button_data_name,
red_button_conf.param,
red_button_conf.value
FROM `red_button_conf`
WHERE " . $where . "
ORDER BY " . $orderBy . "
" . (($_VALID['limit']) ? "
...
<header class="page-header">
    <!-- <h2><?php echo ss('Red_button_conf'); ?></h2> -->
...
<section class="panel">
    <header class="panel-heading">
        ...
        <h2 class="panel-title"><?php echo ss('Red_button_conf'); ?></h2>
    </header>
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="mb-md text-left">
                    <a class="btn btn-success" href="red_button_conf_d.php" title=" <?php echo ss('Add new'); ?>"><i class="fa fa-plus"></i> <?php echo ss('Add new'); ?></a>
                    <!-- Here goes nothing -->
                </div>
            </div>
            <div class="col-sm-12 col-md-6">
...
if (!$headless) {
    ...
    // title red_button_conf_id
    echo '<th class="hasmenu grey" id="th_red_button_conf_id" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_red_button_conf_id" onClick="changeSort(\'red_button_conf_id\')">' . ss('Red_button_conf_id') . '</a>
    </th>';
        
    // title red_button_entity_name
    echo '<th class="hasmenu grey" id="th_red_button_entity_name" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_red_button_entity_name" onClick="changeSort(\'red_button_entity_name\')">' . ss('Red_button_entity_name') . '</a>
    </th>';
        
    // title red_button_data_name
    echo '<th class="hasmenu grey" id="th_red_button_data_name" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_red_button_data_name" onClick="changeSort(\'red_button_data_name\')">' . ss('Red_button_data_name') . '</a>
    </th>';
        
    // title param
    echo '<th class="hasmenu grey" id="th_param" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_param" onClick="changeSort(\'param\')">' . ss('Param') . '</a>
    </th>';
        
    // title value
    echo '<th class="hasmenu grey" id="th_value" nowrap="nowrap">
        <a href="javascript:void(0)" id="hlink_value" onClick="changeSort(\'value\')">' . ss('Value') . '</a>
    </th>';
    ...
    // filter red_button_conf_id
    echo '<th nowrap>
    <input class="form-control search" name="red_button_conf_id" id="red_button_conf_id" value="'.$_SESSION[$modul]['red_button_conf_id'].'" autocomplete="off"></th>';

    // filter red_button_entity_name
    echo '<th nowrap>
    <input class="form-control search" name="red_button_entity_name" id="red_button_entity_name" value="'.$_SESSION[$modul]['red_button_entity_name'].'" autocomplete="off"></th>';

    // filter red_button_data_name
    echo '<th nowrap>
    <input class="form-control search" name="red_button_data_name" id="red_button_data_name" value="'.$_SESSION[$modul]['red_button_data_name'].'" autocomplete="off"></th>';

    // filter param
    echo '<th nowrap>
    <input class="form-control search" name="param" id="param" value="'.$_SESSION[$modul]['param'].'" autocomplete="off"></th>';

    // filter value
    echo '<th nowrap>
    <input class="form-control search" name="value" id="value" value="'.$_SESSION[$modul]['value'].'" autocomplete="off"></th>';
...
if (!$listResult) {
    ...
    while($row = mysqli_fetch_array($listResult)) {
        echo '<tr class="dotted ' .  ((($i++ % 2)==0) ? "tr_even":"tr_odd") . '" id="tr_'.$row['red_button_conf_id'].'">';
        echo '<td nowrap class="text-center actions-hover actions-fade">
        <a href="red_button_conf_d.php?i=' . $i . '&amp;red_button_conf_id=' . (int) $row['red_button_conf_id'] . '" title="' . ss('Edit') . '" data-toggle="tooltip" data-placement="top">
        <i style="font-size:20px" class="fa fa-pencil" ></i>
        </a>';
        // people with right to delete see the delete button
        if (R(3)){
            echo '&nbsp;&nbsp;<a href="#" title="' . ss('Delete') . '" onclick="delRow(\''.$row['red_button_conf_id'].'\');" data-toggle="tooltip" data-placement="top">
            <i class="fa fa-trash-o" style="color:red;font-size:20px;"></i></a>';
        }
        echo '</td>';

        // red_button_conf_id
        echo '<td onClick="location.href=\'red_button_conf_d.php?i='.($i-1).'&amp;red_button_conf_id='.$row['red_button_conf_id'].'\'" nowrap>
        ' . str_limit(htmlspecialchars($row['red_button_conf_id'])) . '
        </td>';

        // red_button_entity_name
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['red_button_entity_name'])) . '
        </td>';

        // red_button_data_name
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['red_button_data_name'])) . '
        </td>';

        // param
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['param'])) . '
        </td>';

        // value
        echo '<td nowrap>
        ' . str_limit(htmlspecialchars($row['value'])) . '
        </td>';
    }
}
            ...
            <div class="panel-body">
                <div class="modal-wrapper">
                    ...
                    <div class="modal-text">
                        <p><?php echo ss('Do you really want to delete the Red_button_conf?'); ?></p>
                    </div>
                </div>
            </div>
...
$(function(){
    $(document).contextmenu({
        ...
        select: function(event, ui) {
            ...
            switch(ui.cmd){
                case "add":
                    window.location.href = 'red_button_entity_d.php?return_script=<?php echo urlencode($_SERVER['PHP_SELF'].'?BRB')?>&entity_name=red_button_conf&after=' + $target.uniqueId().attr('id').substring(3);
                    break
                case "delete":
                    $.ajax({
                        url: 'a/red_button_entity_del.php?searchid&entity_name=red_button_conf&data_name='+$target.uniqueId().attr('id').substring(3)
                    }).done( function( data ) {
                        $.ajax({
                            url: 'bigredbutton.php?array=red_button_conf&http_method=_REQUEST&is_code_type=0&type=form&is_file=0&hrose_write_file=on&hrose_overwrite_file=on&hrose_overwrite_changed_file=on&is_options=0&charset=UTF8&trans=on&mysql_typecast=on&mysql_full=on&validate_store_session=on&type_filter=on&custom_html_tags&form_full=on&list_search=on&list_sort=on&list_limit=on&list_delete=on&list_ajax=on&dynamic_list_header=on&list_odd_row=on&submitted=yes'
                        }).done( function( data ) {
                            $.ajax({
                                url: 'bigredbutton.php?array=red_button_conf&http_method=_REQUEST&is_code_type=0&type=list&is_file=0&hrose_write_file=on&hrose_overwrite_file=on&hrose_overwrite_changed_file=on&is_options=0&charset=UTF8&trans=on&mysql_typecast=on&mysql_full=on&validate_store_session=on&type_filter=on&custom_html_tags&form_full=on&list_search=on&list_sort=on&list_limit=on&list_delete=on&list_ajax=on&dynamic_list_header=on&list_odd_row=on&submitted=yes'
                            }).done( function( data ) {
                                location.reload();
                                return false;
                            });
    ...
    function delRow(pk) {
        ...
        $(document).on('click', '#modalDelete .modal-confirm', function (e) {
            e.preventDefault();
            fetch('red_button_conf_d.php?delete=1&red_button_conf_id='+pk)
            .then(function(response){
                $('#tr_'+pk).hide();
                new PNotify({
                    title: 'Success!',
                    text: '<?php echo ss('The specific red_button_conf deleted!'); ?>',
                    type: 'success'
                });
                $(document).off('click','#modalDelete .modal-dismiss');
                $(document).off('click','#modalDelete .modal-confirm');
...
var sortcol = 'red_button_conf_id';
var sortdir = '';
var del = '';
function updateList() {
...
function autocompleteUpdate(elem) {
    ...
    $.ajax({
    url: 'red_button_conf_d.php?list_update',
    type: 'post',
    data: { field:field_name, [field_name]:value, red_button_conf_id:edit_id },
  ...
  $(".dotted td").on("click", function() {
    ...
    $( ".autocomplete" ).autocomplete({
       source: function( request , response ) {
        ...
        $.ajax({
            url: "red_button_conf.php?headless=1&autocomplete",
            data : param,
            dataType: "json",
            type: "GET",
...
// hook red_button_conf javascript
</script>
<style type="text/css" >
#loading-image {
