...
$n4a['ih_repo_d.php'] = '' . ss('Add Ih_repo') . '';
...
$_SESSION[$modul]['username'] = $_VALID['username'];
...
if (isset($_REQUEST['delete'])) {
	$sql = "DELETE FROM ih_repo WHERE ih_repo_id = " . (int) $_REQUEST['ih_repo_id'];
	mysqli_query($con, $sql) or error_log(mysqli_error($con));
	/*** After Delete Query ***/
	require MODULE_ROOT . 'ih/repo_delete.inc.php';
}

// where condition
if ($_VALID['ih_repo_id']) {
	$where[] = "ih_repo.ih_repo_id =  " . $_VALIDDB['ih_repo_id'];
}
$_SESSION[$modul]['ih_repo_id'] = $_VALID['ih_repo_id'];
if ($_VALID['username']) {
	$where[] = "ih_repo.username LIKE '%" . mysqli_real_escape_string($con, $_VALID['username']) . "%'";
}
$_SESSION[$modul]['username'] = $_VALID['username'];
if ($_VALID['password']) {
	$where[] = "ih_repo.password LIKE '%" . mysqli_real_escape_string($con, $_VALID['password']) . "%'";
}
$_SESSION[$modul]['password'] = $_VALID['password'];
...
require MODULE_ROOT . 'ih/repo_list_result.inc.php';

    if (!$headless) {
    ?>
    <header class="page-header">
        <!-- <h2><?php echo ss('Ih_repo'); ?></h2> -->
    ...
    if (!$headless) {
        echo '<thead>';
        echo '<tr class="head">';echo '<th class="text-center">' . ss('Action') . '</th>';echo '<th class="hasmenu grey" id="th_ih_repo_id" nowrap="nowrap">' . ss('Ih_repo_id') . '</th>';
echo '<th class="hasmenu grey" id="th_username" nowrap="nowrap">' . ss('Username') . '</th>';
echo '<th class="hasmenu grey" id="th_password" nowrap="nowrap">' . ss('Password') . '</th>';
echo '</tr>';echo '<tr class="head">';echo '<th class="text-center" style="vertical-align:middle;font-size:20px;"><span onclick="resetFilter();" style="cursor: pointer; cursor: hand;color:red;"title="' . ss('Reset Filter') . '"><i class="fa fa-filter"/></span></th>';echo '<th></th>';echo '<th></th>';echo '<th></th>';echo '</tr></thead><tbody class="list-body" id="list_tbody">';}
    ...
    if (!$listResult) {
        ...
        for ($i = 0; $i < $cnt; $i++) {
        $row = $listResult[$i];
        echo '<tr class="dotted ' .  ((($i % 2)==0) ? "tr_even":"tr_odd") . '" id="tr_'.$i.'">';
            echo '<td nowrap class="text-center actions-hover actions-fade"><a href="ih_repo_d.php?i=&amp;" title="' . ss('Edit') . '" data-toggle="tooltip" data-placement="top"><i style="font-size:20px" class="fa fa-pencil" ></i></a>';
        // people with right to delete see the delete button
            if (R(3)){
                    echo '&nbsp;&nbsp;<a href="#" title="' . ss('Delete') . '" onclick="if (confirm(\'' . ss('Do you really want to delete the Ih_repo?') . '\')) delRow(\''.$row['ih_repo_id'].'\');" data-toggle="tooltip" data-placement="top">
                    <i class="fa fa-trash-o" style="color:red;font-size:20px;"/></a>';}
                    echo '</td>';
			echo '<td onClick="location.href=\'ih_branch.php?i='.($i-1).'&amp;ih_repo_id='.$row['ih_repo_id'].'\'" nowrap>' . str_limit(htmlspecialchars($row['ih_repo_id']), 600) . '</td>';
			echo '<td nowrap>' . str_limit(htmlspecialchars($row['username'])) . '</td>';
			echo '<td nowrap>' . str_limit(htmlspecialchars($row['password'])) . '</td>';}
}
